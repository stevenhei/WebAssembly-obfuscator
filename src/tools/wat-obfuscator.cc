...
static double inject_rate;
static bool enabled_inject_rate;
static bool enabled_junk_code_injection;
...
R"(  read a file in the wasm text format, obfuscate the file, and
  convert it to the wasm binary format.

examples:
  # parse and typecheck test.wat
  $ wat-obfuscator test.wat

  # parse test.wat and write to binary file test.wasm
  $ wat-obfuscator test.wat -o test.wasm

  # parse spec-test.wast, and write verbose output to stdout (including
  # the meaning of every byte)
  $ wat-obfuscator spec-test.wast -v
)";

static void ParseOptions(int argc, char* argv[]) {
  OptionParser parser("wat-obfuscator", s_description);
  ...
  parser.AddOption('i',"injectrate", "DOUBLE", "Injection rate of garbage code(default=0.5)",
                   [](const char* argument) { enabled_inject_rate = true; inject_rate = strtod(argument, NULL); });
  parser.AddOption("junkcodeinjection", "Enable Junk Code Insertion",
                   []() { enabled_junk_code_injection = true;});
...
int ProgramMain(int argc, char** argv) {
  ...
  if (Succeeded(result) && s_validate) {
   ...
   //Run obfuscations
   if (enabled_junk_code_injection || enabled_inject_rate){
    if (enabled_inject_rate){
      JunkCodeInsertion junkCodeInsertion(inject_rate);
      junkCodeInsertion.Obfuscate(&module); 
    }else{
