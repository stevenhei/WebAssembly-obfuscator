...
static double insert_rate;
static bool enabled_insert_rate;
...
static void ParseOptions(int argc, char* argv[]) {
  ...
  parser.AddOption('i',"insertrate", "DOUBLE", "Insertion rate of garbage code(default=0.5)",
                   [](const char* argument) { enabled_insert_rate = true; insert_rate = strtod(argument, NULL); });
...
int ProgramMain(int argc, char** argv) {
  ...
  if (Succeeded(result) && s_validate) {
   ...
   //Obfuscator takes place
   if (enabled_insert_rate){
     Obfuscator obfuscator(insert_rate);
